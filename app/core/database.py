...
def get_database_status() -> tuple[bool, str]:
    ...
    try:
        ...
        if expected_db and connected_db != expected_db:
            return (
                False,
                f"Connected to unexpected DB '{connected_db}' (expected '{expected_db}')",
            )

        return (
            True,
            f"Postgres connected successfully: db={connected_db}, user={row['db_user']}, host={row['db_host']}, port={row['db_port']}",
        )
    except Exception as exc:
        return False, f"Failed to connect to Postgres: {exc}"